...
const TrustSection = () => {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-white to-[#e8f9fd]/30 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(72,219,251,0.08),transparent)]"></div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-20 animate-fade-in">
          <div className="inline-block bg-gradient-to-r from-[#48dbfb]/10 to-[#38b9e0]/10 text-[#2698b8] px-6 py-2 rounded-full text-sm font-semibold mb-6">
            Kundenzufriedenheit
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-20">
          {trustIndicators.map((indicator, index) => {
            ...
            return (
              <div 
                key={index}
                className="group bg-white/80 backdrop-blur-md border border-[#b3f0ff]/50 rounded-2xl p-4 sm:p-6 text-center shadow-md hover:shadow-lg hover:-translate-y-1 hover:bg-white/90 transition-all duration-500 animate-fade-in overflow-hidden"
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-20">
          {testimonials.map((testimonial, index) => (
            <div 
              key={index}
              className="group bg-white/85 backdrop-blur-md border border-[#b3f0ff]/50 rounded-3xl p-8 shadow-md hover:shadow-xl hover:shadow-[#48dbfb]/5 hover:-translate-y-2 hover:bg-white/93 transition-all duration-700 animate-fade-in"
              style={{ animationDelay: `${0.4 + index * 0.15}s` }}
            >
              {/* Quote Background */}
              <div className="relative">
                <div className="absolute -top-2 -left-2 w-8 h-8 bg-gradient-to-br from-[#48dbfb]/20 to-[#38b9e0]/20 rounded-full flex items-center justify-center">
                  <Quote className="w-4 h-4 text-[#48dbfb]" />
                </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-gradient-to-br from-[#48dbfb]/20 to-[#38b9e0]/20 rounded-full flex items-center justify-center font-semibold text-[#2698b8]">
                    {testimonial.initial}
                  </div>
        ...
        <div className="bg-gradient-to-r from-[#e8f9fd]/30 to-white/50 rounded-3xl border border-[#b3f0ff]/30 p-8 md:p-12 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            {securityBadges.map((badge, index) => (
              <div 
                key={index}
                className="group p-3 sm:p-4 rounded-2xl border border-[#b3f0ff]/50 bg-white/80 backdrop-blur-sm shadow-sm hover:bg-white/90 hover:shadow-md transition-all duration-300 animate-fade-in overflow-hidden"
                style={{ animationDelay: `${0.8 + index * 0.1}s` }}
              >
                <div className="text-lg sm:text-xl font-bold text-gray-800 mb-2 group-hover:text-[#48dbfb] transition-colors duration-300 break-words">
                  {badge.name}
                </div>
